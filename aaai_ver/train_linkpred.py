...
for i in range(1):
    ...
    # write result every trial
        file_name = f"./{args.dataset.lower()}_nohup_outfiles/link/{round(np.mean(AUC_test_list+AP_test_list) * 100, 2)}_{args.ncaps}_{unique_id}.txt"
    ...
    if best_test_acc < np.mean(test_list):
        ...
        with open(f'./{args.dataset.lower()}_nohup_outfiles/link_besthyper_{unique_id}.txt','w') as file:

...
class MaskGAE(nn.Module):
    def __init__(
        ...
        self.previous_unconflicted = [0]*10
        # clustring
        ...
        self.beta1 = 0.4 #for Cite   #cora:0.4
        self.beta2 = 0.1 #for Cite  #cora:0.10   5 2  140 eopch
        self.cluster_pred=0
    ...
    def cluster_loss(self, args, q_unconf, p_unconf, q, y, label_unconflict_index):

        loss_node = F.cross_entropy(q[label_unconflict_index], y[label_unconflict_index])
        loss_clus = self.kl_loss(torch.log(q_unconf), p_unconf)

        loss = 2*loss_node + 0.001 * loss_clus
        # print('loss_node', loss_node)
        # print('loss clus', 0.001*loss_clus)
        # loss = 2*loss_node + 0.0005 * loss_clus
        return loss_clus
    ...
    def train_epoch(
        ...
        self.ncluster = 20 # len(torch.unique(y))
        ...
        for perm in DataLoader(
            ...
            if epoch % 15 == 0:
                ...
                # print('p',self.p,self.p.shape)
                # print('self.unconflicted_ind',self.unconflicted_ind,self.unconflicted_ind.shape)
                # print('self.self.conflicted_ind',self.conflicted_ind,self.conflicted_ind.shape)
            ...
            if len(self.previous_unconflicted) > 5:
                ...
                print('35 Accuracy with train_mask:', accuracy2.item(), 'how many:', len(self.previous_unconflicted))
            ...
            print('\nedge_loss', edge_loss, 'cluster_loss',cluster_loss, 'ch_loss', ch_loss)
            #loss = edge_loss #+ alpha*cluster_loss #+ 0* ch_loss
            # ******************************************************************
            loss.backward()

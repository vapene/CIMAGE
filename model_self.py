...
class MaskGAE(nn.Module):
    ...
    def clustering_loss(self, z, edge_index, trace_reg = 0.1):
        ## making B (modularity matrix)
        z = torch.nn.functional.normalize(z, p=2, dim=1)
        dense_adj = to_dense_adj(edge_index)[0]
        degree_mat = torch.sum(dense_adj, dim=1)
        degree_mat = degree_mat.reshape(-1, 1)
        degree_mult = torch.matmul(degree_mat, degree_mat.T) # N * N matrix
        edge_num = len(edge_index[0]) # check need
        modularity_mat = dense_adj - degree_mult / (2* edge_num)
        
        ### clustering loss-1. trace
        trace_mat = torch.matmul(modularity_mat, z)
        trace_mat = torch.matmul(trace_mat, z.T) # N * N matrix

        # scatter_mat
        node_num = trace_mat.shape[0]
        scatter_mat = torch.scatter(
    ...
    def train_epoch(
        ...
        for perm in DataLoader(
            ...
            new_cluster_filtered = []
            for idx in range(new_cluster[0].size(0)):
                i = new_cluster[0][idx].item()
                j = new_cluster[1][idx].item()
                if i in previous_unconflicted_set and j not in previous_unconflicted_set:
                    new_cluster_filtered.append(j)
            previous_unconflicted_set.update(new_cluster_filtered)
            self.previous_unconflicted_new = torch.tensor(list(previous_unconflicted_set), device=self.previous_unconflicted.device)

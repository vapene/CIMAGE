...
def rbf_mine(device, X_in_1, X_in_2, sigma=1.0):
    return torch.exp( -(torch.cdist(X_in_1.T, X_in_2.T, p=2, compute_mode='donot_use_mm_for_euclid_dist') ** 2) / (2 * sigma ** 2))
...
def compute_kernel(device, x, kernel, B=0, M=1, discarded=0):
    ...
    for m in range(M):
        np.random.seed(m)
        index = torch.from_numpy(np.random.permutation(index))
        # torch.manual_seed(m)
        ...
        for i in range(0, n - discarded, B):
            j = min(n, i + B)
            if kernel == 'Gaussian':
                k = kernel_gaussian(device, x[:, index[i:j]], x[:, index[i:j]], torch.sqrt(torch.Tensor(d)).to(device))

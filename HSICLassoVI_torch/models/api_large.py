...
def kernel_gaussian(device, X_in_1, X_in_2, sigma=1.0):
    return rbf_mine(device, X_in_1, X_in_2, sigma=sigma)


def compute_kernel(device, x, kernel, B=0, M=1, discarded=0):
    _, n = x.shape

    H = torch.eye(B, device=device, dtype=torch.float32) - (1.0 / B)
    K = torch.zeros(n * B * M, device=device, dtype=torch.float32)

    if kernel in ['Gaussian']:
        x = x / (x.std() + 1e-20)
        torch.manual_seed(0)
        index = torch.randperm(n, device=device).split(B)
        for m, idx in enumerate(index):
            if len(idx) == B:
                selected_x = x[:, idx]
                k = kernel_gaussian(device, selected_x, selected_x, torch.sqrt(d).item())
                k = torch.einsum('ij,jk,kl->il', [H, k, H])
                K[m * B ** 2:(m + 1) * B ** 2] = (k / (torch.linalg.norm(k, 'fro') + 1e-10)).flatten()

    # Extend this pattern for other kernels if needed
    else:
...
def make_kernel(device, X, Y, y_kernel, x_kernel='Gaussian', n_jobs=-1, discarded=0, B=0, M=1):
    d, n = X.shape
    L = compute_kernel(device, Y, y_kernel, B, M, discarded).reshape(n * B * M, 1)

    with ThreadPoolExecutor(max_workers=n_jobs) as executor:
        results = list(executor.map(lambda k: (k, compute_kernel(device, X[k].unsqueeze(0), x_kernel, B, M, discarded)),
                                    range(d)))

    K = torch.stack([res[1] for res in sorted(results, key=lambda x: x[0])]).transpose(0, 1)
    KtL = torch.matmul(K.transpose(0, 1), L)
    return K, KtL, L
...
class Proposed_HSIC_Lasso(object):
    ...
    def classification_multi(self, B=20, M=3, kernels=['Gaussian']):
    ...
    def _run_hsic_lasso_multi(self, B, M, kernels=['Gaussian'], y_kernel='Gaussian'):
        ...
        for kernel in kernels:
            _X, _Xty, _Ky = make_kernel(self.device, self.X_in, self.Y_in, y_kernel, kernel,

...
def rbf_mine1(device, X_in_1, X_in_2, sigma=1.0):
    return torch.exp(
...
def kernel_gaussian(device, X_in_1, X_in_2, sigma=1.0):
    aaa1 = rbf_mine1(device, X_in_1, X_in_2, sigma=1.0)
    return aaa1

def make_kernel(device, X, Y, y_kernel, x_kernel='Gaussian', n_jobs=-1, discarded=0, B=0, M=1):
    d, n = X.shape  # 512, train_indx    32, 949
    #dy = Y.shape[0]
    L = compute_kernel(device, Y, y_kernel, B, M, discarded) # [939]
    L = torch.reshape(L, (n * B * M, 1)) # [939,1]   n=939, B=20,M=3
    ######## BY GROUP
    # group_size = 32  # Size of each group
    # num_groups = d // group_size  # Number of groups
    # result = Parallel(n_jobs=n_jobs)([delayed(parallel_compute_kernel)(device,torch.reshape(X[k * group_size:(k + 1) * group_size, :],(group_size, n)), x_kernel, k, B, M, n, discarded) for k in range(num_groups)])
    # result = dict(result)
    # K = torch.stack([result[k] for k in range(num_groups)]).reshape(-1, group_size).T
    # # K shape (28170x32 and L shape 56340x1)
    ######### BY MEAN
    result = Parallel(n_jobs=n_jobs)([delayed(parallel_compute_kernel)(device,torch.reshape(X[k, :], (1, n)), x_kernel, k, B,M, n, discarded) for k in range(d)])
    result = dict(result)

    K = torch.stack([result[k] for k in range(d)]).T # [939, 32]

    ##############
    KtL = torch.matmul(K.T, L) # torch.Size([32, 1])
    return K, KtL, L


def compute_kernel(device, x, kernel, B=0, M=1, discarded=0):
    d, n = x.shape  # 32, 939     # for L 1,939
    ...
    for m in range(M):
        ...
        # np.random.seed(m)
        # index = torch.arange(n)
        # num_elements = len(index)
        # group_size = 32
        # num_groups = num_elements // group_size
        # group_indices = np.arange(num_groups)
        # np.random.shuffle(group_indices)
        # shuffled_index = np.array([i * group_size + j for i in group_indices for j in range(group_size)])
        # index = torch.tensor(shuffled_index)
...
class Proposed_HSIC_Lasso(object):
    ...
    def _run_hsic_lasso_multi(self, B, M, n_jobs, kernels=['Gaussian'], y_kernel='Gaussian'):
        ...
        for kernel in kernels:
            ...
            # print('_X',_X.shape) # torch.Size([939, 32])
            # print('_Xty',_Xty.shape) # torch.Size([32, 1])
            # print('_Ky',_Ky.shape) # torch.Size([939, 1])
            X.append(_X)
            Xty.append(_Xty)
            Ky.append(_Ky)
        ...
        # print('X',X.shape) # X torch.Size([939, 32, 1])
        # print('Xty',Xty.shape) # Xty torch.Size([32, 1, 1])
        # print('Ky',Ky.shape) # Ky torch.Size([939, 1])
        ...
        # print('\n mine self.KX',self.KX)
        # print('\n mine self.Ky',self.Ky)

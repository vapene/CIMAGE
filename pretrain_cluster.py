...
from model_pretrain_cluster import MaskGAE, MLPEncoder, EdgeDecoder, ChannelDecoder
...
def train_linkpred(run, model, splits, args, device="cpu"):
    def train(data, epoch):
        model.train()
        print('train_link')
        edge_loss, non_zero, z = model.train_epoch(args, data.to(device), optimizer, scheduler, previous_unconflicted, previous_conflicted, batch_size=args.batch_size,
    ...
    # load only edge
    # checkpoint = torch.load(f'/home/jongwon208/MaskGAE/kdd_ver/pretrained/{args.dataset}_onlyedge_scheduler_seed0.pt')
    # #
    # # # load all
    # model.load_state_dict(checkpoint['model_state_dict'])
    # model.previous_unconflicted = checkpoint['previous_unconflicted']
    # model.cluster_pred = checkpoint['cluster_pred']
    ...
    for epoch in range(args.epochs):
        ...
        # test node
        # node_val, node_test, node_loss_list, node_acc_val_list, node_acc_test_list = train_nodeclas(run, model, data,
        #                                                                                             args, device=device)
...
parser.add_argument('--hsic', type=bool, default=False, help='do disentangle?')
...
if use_best:   # /home/jongwon208/MaskGAE/mine_encoder_list/configs.yml
    args = load_best_configs(args, "cluster_configs.yml")
else:
    ...
    for run in range(runs):
        ...
        def sample_data(data, num_samples_per_class):
            ...
            for label in torch.unique(train_labels):
                label_indices = train_nodes[train_labels == label]
                sampled_label_indices = label_indices[torch.randperm(len(label_indices))[:num_samples_per_class]]
                sampled_indices.append(sampled_label_indices)
            sampled_indices = torch.cat(sampled_indices)
            sample_train_mask = torch.zeros_like(data.y, dtype=bool)
            sample_train_mask[sampled_indices] = True
            return sample_train_mask


        print('oroginal train)mask', sum(data.train_mask), data.x.shape)
        # samples_per_class=1
        # sample_train_mask = sample_data(data, num_samples_per_class=samples_per_class)
        # save sample train_mask.
        # torch.save(sample_train_mask, f'./pretrained/{args.dataset}_1shot_train_index.pt')
        # exit()
        # data.train_mask = sample_train_mask
        # print('sampled train)mask', sum(data.train_mask))

        # percent sample
        # if not args.hsic:
        #     print('only use 1%')
        #     total_data = len(data.train_mask)
        #     total_labels = len(torch.unique(data.y))
        #
        #     percentage = 1/100
        #     total_samples_to_use = int(total_data * percentage)
        #     samples_per_class = total_samples_to_use // total_labels
        #     # print('total', total_data, total_labels, samples_per_class)
        #     sample_train_mask = sample_data(data, num_samples_per_class=samples_per_class)
        #
        #     data.train_mask = sample_train_mask
        # print('sampled train)mask', sum(data.train_mask))
        ...
        model = MaskGAE(args, encoder, edge_decoder,ch_decoder, mask, torch_generator=th_g, num_labels = 20).to(device)

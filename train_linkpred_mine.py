...
from utils import load_dataset, set_seed, load_best_configs, seed_worker, set_args_link, find_number_with_no_remainder
from model_ver1 import MaskGAE, MLPEncoder, EdgeDecoder, ChannelDecoder
...
def train_linkpred(run, model, splits, args, device="cpu"):
    def train(train_data, epoch):
        model.train()
        edge_loss, non_zero = model.train_epoch(args, train_data.to(device), optimizer, scheduler, batch_size=args.batch_size,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2,
                                                                     eta_min=args.link_lr_min, last_epoch=-1)
    cheating_test = 0
    for epoch in range(1, 1 + args.epochs):
        edge_loss = train(splits['train'], epoch)

        results = test(epoch, splits)
        valid_result = (results['AUC'][0]+results['AP'][0])/2

        if valid_result > best_valid:
            best_valid = valid_result
            best_epoch = epoch
            try:
                os.remove('./saved_models/' + unique_id + best_time + save_path)
            except:
                pass
            best_time = str(time.strftime("%Y%m%d-%H%M%S"))
            torch.save(model.state_dict(), './saved_models/' + unique_id + best_time + save_path)
            cnt_wait = 0
        else:
            cnt_wait += 1

        print(f"epoch:{epoch}, Best epoch:{best_epoch}, valid: {valid_result}, best_valid: {best_valid}, cheating_testL {cheating_test}")
        ...
        if cnt_wait == args.patience:
            ...
            break
    model.load_state_dict(torch.load('./saved_models/' + unique_id + best_time + save_path))
    os.remove('./saved_models/' + unique_id + best_time + save_path)
    results = test(best_epoch, splits)
    for key, result in results.items():
        valid_result, test_result = result
        print(f'link {key}, '
              f'Valid: {valid_result:.2%}, '
              f'Test: {test_result:.2%}')
    return results, link_loss_list, link_AUC_valid_list, link_AUC_test_list, link_AP_valid_list, link_AP_test_list, non_zero_list
...
def train_nodeclas(run, model, data, args, device='cpu'):
    ...
    print(f'##### Testing result for Node Classification',
          ...
          f'Valid: {best_val:.2%}, '
          f'Test: {best_test:.2%}')
    return best_val, best_test, node_loss_list, node_acc_val_list, node_acc_test_list


parser = argparse.ArgumentParser()
parser.add_argument("--seed", type=int, default=0, help="None")
...
parser.add_argument('--ncaps', type=int, default=32, help='num channels. (default: 4)')
...
parser.add_argument('--original_lamb', type=float, default=0.7, help='Penalty on original A(default: 0.1)')
parser.add_argument('--new_lamb', type=float, default=1.0, help='Penalty on new A(default: 0.1)')
parser.add_argument('--recon_alpha', type=float, default=0.4, help='Penalty on new A(default: 1.0)')
...
parser.add_argument('--alpha_l', type=int, default=3, help='(pubmed 3)')
...
parser.add_argument('--patience', type=int, default=300, help='(default: 4)')
...
if use_best:   # /home/jongwon208/MaskGAE/mine_encoder_list/configs.yml
    args = load_best_configs(args, "link_configs.yml")
else:
...
for i in range(1):
    ...
    for run in range(runs):
        ...
        # if args.dataset in ["Cora", "Citeseer", "Pubmed", "Photo", "Computers", "CoraFull", "WikiCS"]:
        train_B = sum(data.train_mask).detach().cpu()
        # else:
        #     train_B = min(sum(data.train_mask).detach().cpu(),
        #                   find_number_with_no_remainder(sum(data.train_mask).detach().cpu()))

        valid_B = min(sum(data.val_mask).detach().cpu(),
                      find_number_with_no_remainder(sum(data.val_mask).detach().cpu()))
        test_B = min(sum(data.test_mask).detach().cpu(),
                     find_number_with_no_remainder(sum(data.test_mask).detach().cpu()))
        ...
        model = MaskGAE(args, encoder, edge_decoder,ch_decoder, mask, torch_generator=th_g).to(device)
        # load train_neighbors &  neighbors
        # if args.dataset=='arxiv':
        #     train_neighbors = torch.load("/home/jongwon208/public_data/pyg_data/ogbn_arxiv/train_neighbors.pt")
        #     neighbors = torch.load("/home/jongwon208/public_data/pyg_data/ogbn_arxiv/neighbors.pt")
        # else:
        train_neighbors = model.neigh_sampler_torch(args, splits['train'].num_nodes, splits['train'].edge_index, args.seed)
        neighbors = model.neigh_sampler_torch(args, data.num_nodes, data.edge_index, args.seed)  # 이거 항상 같음. 저장해놓자.

        print('\n start link pred')

        results, link_loss_list, link_AUC_valid_list, link_AUC_test_list, link_AP_valid_list, link_AP_test_list, non_zero_list = train_linkpred(run, model, splits, args, device=device)

        node_val, node_test, node_loss_list, node_acc_val_list, node_acc_test_list = train_nodeclas(run, model, data, args, device=device)
        ...
        del model
        del splits
        del valid_pos_edge_label_index
        del valid_neg_edge_label_index
        del test_pos_edge_label_index
        del test_neg_edge_label_index
        del train_neighbors
        del neighbors
        del edge_decoder
        del ch_decoder
        ...
        if args.dataset=='Pubmed':
            if results['AUC'][1] < 0.987:
                break
    ####
    keep_log = True

    # if args.dataset == 'Cora':
    #     if np.mean(test_list) < 0.84:
    #         keep_log = False
    # elif args.dataset == 'Citeseer':
    #     if np.mean(test_list) < 0.74:
    #         keep_log = False

    if keep_log == True:
    # write result every trial
        file_name = f"/home/jongwon208/MaskGAE/mine_encoder_list/{args.dataset.lower()}_nohup_outfiles/link/{round(np.mean(AUC_test_list+AP_test_list) * 100, 2)}_{args.ncaps}_{unique_id}.txt"
        with open(file_name, 'w') as file:
            file.write("logs of link prediction:\n"
        ...
        print('try again. Low test',np.mean(test_list))
    ...
    if best_test_acc < np.mean(test_list):
        ...
        with open(f'/home/jongwon208/MaskGAE/mine_encoder_list/{args.dataset.lower()}_nohup_outfiles/link_besthyper_{unique_id}.txt','w') as file:

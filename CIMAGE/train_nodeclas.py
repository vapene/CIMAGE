...
def train_linkpred(run, model, splits, args, device="cpu"):
    def train(data, epoch):
        model.train()
        print('train_link')
        edge_loss, non_zero = model.train_epoch(args, data.to(device), optimizer, scheduler, batch_size=args.batch_size,
...
def train_nodeclas(run, model, data, args, device='cpu'):
    ...
    scheduler_node = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer_node, T_0=10, T_mult=2,eta_min=args.node_lr_min, last_epoch=-1)
...
for i in range(1):
    ...
    for run in range(runs):
        ...
        gc.collect()
        torch.cuda.empty_cache()
        if (args.dataset == "Photo") and (node_test < 0.923):
            break
        elif (args.dataset == "Computers") and (node_test < 0.87):
            break
